...
![Time to Change — explainable market timing in Telegram](./assets/time-to-change-hero.svg)
...
- Ведёт персональную историю алертов и утренний дайджест; владелец инстанса может дополнительно вести бюджетный план конвертации EUR → USD.
...
| `/history` | Показывает последние 10 алертов по подпискам текущего пользователя |
...
| `/budget 6000 30d` | Только владелец: ставит цель конвертации EUR → USD |
| `/budget done 1500 1.0852` | Только владелец: записывает частичную конвертацию |
| `/undo` | Только владелец: отменяет последнюю запись конвертации |
| `/users` | Только владелец: показывает пользователей инстанса |
| `/leave` | Удаляет участника и его подписки; владелец не может случайно выйти |

Любой пользователь может заглушить уведомления на 1 или 7 дней. Кнопки записи конвертации и все детали бюджета видит только владелец инстанса; участники получают рыночную аналитику без доступа к его финансовому плану.
...
- `ci.yml` проверяет TypeScript Worker и Python reference implementation при каждом PR и push в `main`; используемые Actions закреплены за неизменяемыми commit SHA.
- `deploy-worker.yml` вручную запускает полный Worker-gate, миграции, deploy и минимальный health-check через защищённый GitHub Environment `production`.
- `backtest.yml` вручную строит отчёт на исторических данных.
- `d1-backup.yml` еженедельно экспортирует полное состояние D1, шифрует архив AES-256-CBC + PBKDF2, проверяет обратную расшифровку и структуру архива и только после этого загружает artifact. Он включается переменной `D1_BACKUP_ENABLED=true` и использует secrets из environment `production`.
- Vulnerability alerts остаются включены, а автоматические dependency-PR намеренно отключены: стабильный self-hosted проект не шумит, обновления проверяются вручную.
...
- Структурированные логи отбрасывают Telegram-идентификаторы, имена, callback ID, сырые ошибки, URL и поля, похожие на credentials.
- Публичный `/health` отдаёт только версию схемы и свежесть анализа — без пользователей, квот, update ID и сырых ошибок.
- GitHub Actions получает минимальные permissions и использует неизменяемые SHA; публичный fork не может использовать секреты исходного репозитория.
